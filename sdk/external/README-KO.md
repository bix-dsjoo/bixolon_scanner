...
SDK와 Store Model의 version은 독립적입니다. SDK `1.2.1`은 `worker_runtime_schema=2.0`과
`catalog_schema=2.0`을 만족하는 Store Model `0.1.16`, `0.2.1` 등을 선택해서 실행할 수 있습니다.
...
```text
artifacts/external-sdk/1.2.1/BIXOLON-Scanner-SDK-Windows-x64-1.2.1.zip
artifacts/store-models/three_bakery/0.2.1/BIXOLON-Store-Model-three_bakery-0.2.1.zip
```
...
final layout = BixolonRuntimeLayout.discover(
  storeBundleRoot:
      r'C:\ProgramData\BIXOLON\Scanner\bundles\three_bakery\0.2.1',
);
...
Store Model 0.2.1의 `worker-profile.json`에는 CPU Detector 8 / Embedder 12 threads를 기록합니다. SDK Core 1.2.1은 CPU 시작 시 이를 읽으며, 명시한 `extraEnvironment`가 우선합니다. 이전 번들에 파일이 없으면 기존 Worker 기본값을 유지합니다.
