# BIXOLON Bakery AI Scanner 0.1.13 번들

`configs/versions/0.1.13.json`이 배포 조합의 유일한 기준입니다. Python·Worker·Detector·Embedder·
판정 정책·Catalog와 Windows ProductVersion은 `0.1.13`, Flutter 내부 빌드는 `0.1.13+16`입니다.

0.1.13은 매장과 SKU가 달라도 같은 모델 순서와 전역 threshold를 사용하는 것을 우선합니다.
...
![0.1.13 추론 파이프라인](../assets/scanner-0.1.13-inference-pipeline.svg)
...
`scripts/build_app.ps1 -Version 0.1.13`은 Runtime/Catalog/CUDA와 평가 증빙의 고정 해시를 확인하고
binary payload를 바꾸지 않은 채 공개 실행 구성요소 version만 `0.1.13`으로 맞춥니다. 최종 번들은
`version.json`, `provenance.json`, `bundle-manifest.json`과 Runtime metadata가 선언한 license 파일을
포함합니다.

API와 null 규칙은 [Worker 연동 명세](../contracts/worker-integration-0.1.13.md), 후보 비교와 평가
