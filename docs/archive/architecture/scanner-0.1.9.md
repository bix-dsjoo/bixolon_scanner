# BIXOLON Bakery AI Scanner 0.1.9 번들

`configs/versions/0.1.9.json`이 운영 조합의 유일한 기준입니다. Python·Worker·Detector·Embedder·
판정 정책·Catalog와 Windows ProductVersion은 `0.1.9`, Flutter 내부 빌드는 `0.1.9+12`입니다.
`0.1.8`의 payload와 판정 계약을 유지하고, Flutter 빵 원본 촬영 작업공간의 카메라 종횡비와
3×3 가이드 정렬을 수정했습니다.
...
```mermaid
flowchart LR
    CONFIG["configs/versions/0.1.9.json"] --> VERIFY["원본 manifest·증빙 해시 검증"]
    VERIFY --> META["Runtime/Catalog metadata 0.1.9"]
...
`scripts/build_app.ps1 -Version 0.1.9`는 source Runtime/Catalog/CUDA와 평가 증빙 해시를 확인하고,
검증된 source payload를 변경하지 않은 채 실행 구성요소 version만 `0.1.9`로 맞춥니다. 최종 번들은
...
[Worker 연동 명세](../contracts/worker-integration-0.1.9.md)를 따릅니다.
