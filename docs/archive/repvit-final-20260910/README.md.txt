...
별도 제품 **BIXOLON Bakery AI Scanner Lite**는 `0.2.1` CPU Worker를 그대로 사용하며
...
0.2.1은 D-FINE HGNetV2-S 640 class-agnostic detector와 기존 ConvNeXt-Tiny 192/224,
선택적 Frozen ViT-B/16 160·회전 합의를 사용합니다. 검출 점수 0.025 이상 후보를 이웃 마스크의
context로 보존하고, 출력 점수 0.04 이상 ROI만 선택 batch로 분류합니다. 불필요한 조각·중복
출력을 줄이면서 남은 ROI의 전처리·판정과 모델 가중치를 유지합니다.
...
소스 Worker CPU 3회 교차 측정은 정답 APPROVED 1,078개(98.36%)·오승인0·미검출0입니다.
0.2.0 대비 추가 박스37→16개, UNKNOWN17→15·SEGMENT_RECAPTURE38→19개이며,
완전 성공 이미지는99→108/132장입니다. 제거된 21개 박스는 모두 GT 미대응 박스이며
남은 모든 ROI의 상태·품목 순위가 baseline과 같습니다.

CPU HTTP p95는272.14~279.00→239.84~246.79ms이며 모든 교차 측정 쌍에서 개선됐습니다.
현재 장비는 Core Ultra9 285K와 RTX5080이며
N100 실측은 개발 완료 이후 대상 장비에서 수행합니다. N100 내장 Intel UHD용 별도 번들은 CPU검출 →
GPU primary/detail → CPU Frozen ViT·후처리와 명시적 CPU fallback을 구성합니다.
N100 성능은 대상 장비 결과로만 판단합니다.

[0.2.1 변경과 한계](docs/architecture/scanner-0.2.1.md),
[원인 분석·후보 비교](docs/experiments/recapture-0.2.1.md)에 근거를 기록합니다.
배포 파일은 `artifacts/distributions/0.2.1/README-KO.md`, 최종 측정은 같은 폴더의 `RESULTS-KO.md`에 정리합니다.

## 판정 계약

이번 목표는 로그132장 전체 GT 객체 중 정답 승인1,078/1,096개 이상, 오승인0건·미검출0건,
재촬영 최소화와 동일 장비·provider의 0.2.0 대비 HTTP p95 비악화입니다.
