...
`0.1.11`인지 확인한 뒤 scan을 직렬로 전송합니다. JPEG/PNG 한 장만 허용하며 multipart boundary는
...
정상 ROI는 192 입력으로 한 batch 분류합니다. 안전 경계 밖 ROI가 있으면 그 ROI만 224 입력으로
재분류하고 다른 ROI의 결과는 보존합니다. detector class와 classifier의 강한 합의는 metadata의 전역
조건 안에서만 승인 경계에 사용하며, 남은 저신뢰 결과는 `UNKNOWN`+Top-3 또는
`SEGMENT_RECAPTURE`가 됩니다. 공개 reason code와 schema는 이전 계약과 같습니다.
...
[0.1.11 예시](examples/0.1.11/)를 참조하십시오. 빵 식별에는 표시명 대신 `class_id`를 사용합니다.
