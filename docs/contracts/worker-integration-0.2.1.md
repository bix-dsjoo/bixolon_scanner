...
`0.2.1`인지 확인한 뒤 scan을 직렬로 전송합니다. JPEG/PNG 한 장만 허용하며 multipart boundary는
...
단일 class-agnostic D-FINE-S640 detector가 모든 객체 위치와 프레임 품질을 판단합니다. hard 품질 조건이
실패하면 classifier를 호출하지 않고 `IMAGE_RECAPTURE`를 반환합니다. detector ensemble과 별도
`object_presence` 또는 count verifier는 구성하지 않습니다.

정상 ROI 전체를 RepViT-M0.9 192의 정적 batch 2/1로 분류하고 전역 위험 조건에 해당하는 ROI만
...
[0.2.1 예시](examples/0.2.1/)를 참조하십시오. 빵 식별에는 표시명 대신 `class_id`를 사용합니다.
