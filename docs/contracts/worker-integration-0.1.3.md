...
OpenVINO CPU 전달 패키지는 같은 PC의 Windows x64 환경만 지원합니다. `start-worker.ps1`은
`BIXOLON_PROVIDER=openvino`, 요청 제한시간 60초, localhost 전용 바인딩을 설정합니다. 모델 warm-up이
...
신규 SKU Catalog는 기존 `class_id` 순서와 adapter 열을 변경하지 않고 정렬된 뒤쪽에 새 label과
새 adapter 열만 추가합니다. 이 경우 `catalog.json`의 `append_only_base_class_count`는 확장 전
label 수입니다. Worker는 새 SKU가 주 분류기, 180° view 및 독립 verifier에서 모두 Top-1일 때만
확장 결과를 사용하고, 그렇지 않으면 해당 요청의 확장 전 판정·점수·상태를 복원합니다. 이 필드는
`ridge_adapter`와 세 verifier Catalog에만 허용되며 서로 다른 base count나 checksum은 시작 오류입니다.
...
공개된 non-null 버전 값은 모두 같은 제품 버전이어야 합니다. `0.1.3` Worker에서는 실행된
구성요소의 버전도 모두 `0.1.3`입니다.
...
```json
{
  ...
  "detector_version": "0.1.3",
  "classifier_version": "0.1.3",
  "embedder_version": "0.1.3",
  "detector_policy_version": "0.1.3",
  "classifier_policy_version": "0.1.3",
...
구체적인 detector 사유는 공개 응답을 분기하는 값이 아니며 구조화 로그의
`diagnostic_reason_codes`에만 기록합니다.
