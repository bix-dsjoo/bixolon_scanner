...
`46c7395`까지 원격 main에 반영된 것을 확인한 후 대상 release 폴더 5개를 제거했다.
등록된 작업트리 4개와 `codex/release-0.1.17`, `codex/release-0.1.18`,
`codex/release-0.2.0`, `codex/release-0.2.1` 로컬 브랜치도 정리했다.
삭제 직전 원본 소스의 파일 집합과 해시를 스냅샷과 다시 비교했고, junction 및 Flutter
plugin 연결 45개는 대상 내용을 따라가지 않고 연결 항목만 제거했다.

제거된 일반 파일은 14,468개이며 크기 합계는 17,310,053,300 bytes(약 17.3GB)다.
이는 파일의 논리 크기 합계이며 실제 디스크 여유 공간 증가량을 측정한 수치는 아니다.
삭제 후 main에 보존된 0.1.19 산출물 616개와 최종 N100 배포 파일 99개의 해시를 다시
검증했다. 다른 프로젝트, Codex의 다른 작업트리, 원본 데이터, 기존 사용자 로그 및
main의 다른 작업 산출물은 유지했다.
