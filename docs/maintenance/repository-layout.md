...
- `configs/versions/0.1.3.json`: 유일한 활성 제품 설정
...
- `artifacts/packages/bread-scanner-0.1.3-openvino-consensus-runtime`: 활성 Runtime 원본
- `artifacts/catalogs/bread-store-0.1.3-consensus`: 활성 Catalog 원본
- `artifacts/versions/0.1.3`, `artifacts/installers/0.1.3`, `artifacts/handoff/0.1.3`: 재생성 가능한
  운영 전달물
...
`0.1.3` 운영에는 사용하지 않습니다.

정리 전에는 `configs/versions/0.1.3.json`이 가리키는 Runtime, Catalog, CUDA와 평가 증빙이 실제로
