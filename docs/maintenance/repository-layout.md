...
- `configs/versions/0.1.14.json`: 유일한 활성 제품 설정
...
- `artifacts/experiments/yolo-free-0.1.7/runtime-candidate-v42-ssdlite320-selective-fallback`:
  0.1.14 source Runtime
- `artifacts/catalogs/bread-store-0.1.5-consensus`: 0.1.14 source Catalog
- `artifacts/versions/0.1.14`, `artifacts/installers/0.1.14`: 재생성 가능한 정식 배포 산출물
...
provenance입니다.
배포 번들의 Runtime·Catalog metadata와 공개 API 버전은 모두 `0.1.14`로 다시 작성됩니다.
...
정리 전에는 `configs/versions/0.1.14.json`이 가리키는 Runtime, Catalog, CUDA와 평가 증빙이 실제로
