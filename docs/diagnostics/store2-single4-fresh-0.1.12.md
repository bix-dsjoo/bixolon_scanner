...
각 난이도는 이미지 100장으로 구성된다. GT 객체 수는 `easy` 410개, `medium` 500개,
`hard` 500개다. `완전 성공 이미지`는 해당 이미지에 wrong class, FN과 FP가 모두 없는 경우다.

실제 `DecisionPipeline` 최고 기록인 `evaluation-runtime-pipeline-v3`의 결과는 다음과 같다.

| 난이도 | 이미지 | GT | 올바른 `APPROVED` | 정확 승인율 | 잘못된 `APPROVED` | FN | FP | 완전 성공 이미지 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `easy` | 100 | 410 | 410 | 100.0000% | 0 | 0 | 3 | 97 |
| `medium` | 100 | 500 | 498 | 99.6000% | 1 | 1 | 6 | 93 |
| `hard` | 100 | 500 | 490 | 98.0000% | 8 | 2 | 6 | 87 |
| **전체** | **300** | **1,410** | **1,398** | **99.1489%** | **9** | **3** | **15** | **277** |

후속 FN/FP 0 ONNX detector와 원본 중심 fresh classifier를 결합한 구성요소 진단은 다음과 같다.
이 표는 실제 `DecisionPipeline` 실행 결과가 아니며 classifier one-hot 출력은 모두 승인으로
계산했다.

| 난이도 | 이미지 | GT/매칭 | 올바른 class 출력 | 정분류율 | 잘못된 class 출력 | FN | FP | 완전 성공 이미지 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `easy` | 100 | 410/410 | 410 | 100.0000% | 0 | 0 | 0 | 100 |
| `medium` | 100 | 500/500 | 500 | 100.0000% | 0 | 0 | 0 | 100 |
| `hard` | 100 | 500/500 | 487 | 97.4000% | 13 | 0 | 0 | 89 |
| **전체** | **300** | **1,410/1,410** | **1,397** | **99.0780%** | **13** | **0** | **0** | **289** |

후속 구성의 13개 오분류는 모두 `hard`에서 발생했다. 실제 옆면이 부족한 class 2 → 17 오류
3개도 모두 `hard`의 image 226, 248, 267이다. 따라서 detector의 난이도별 recall 문제는 해결됐지만,
실제 옆면·강한 가림·근접 배치가 겹친 `hard` classifier 일반화가 남은 병목이다.
