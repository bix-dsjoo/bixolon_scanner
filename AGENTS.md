...
재촬영 상태는 빨간색으로 표시하며 Worker 0.2.0은 변경 없이 포함합니다.
...
기준은 `configs/versions/0.2.0.json`이며 Python, Worker, Detector, Embedder, Detector policy,
Classifier policy, Catalog와 사용자 표시 버전은 모두 `0.2.0`입니다. Flutter 내부 빌드만
`0.2.0+23`를 사용합니다.
...
4. 활성 `0.2.0` Runtime은 class-agnostic Detector의 정상 ROI 전체를 DINOv3 ConvNeXt-Tiny 192
...
있습니다. 현재 `0.2.0`를 독립 일반화 성능, 인증 또는 SLA 달성으로 표현하지 마십시오.
