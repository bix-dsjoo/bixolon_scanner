...
기준은 `configs/versions/0.1.1.json`이며 Python, Worker, Detector, Embedder, Detector policy,
Classifier policy, Catalog와 사용자 표시 버전은 모두 `0.1.1`입니다. Flutter 내부 빌드만
`0.1.1+3`을 사용합니다.
...
있습니다. 현재 `0.1.1`을 독립 일반화 성능, 인증 또는 SLA 달성으로 표현하지 마십시오.
