...
`APPROVED`의 `prediction` 상품 ID·이름도 읽기 전용으로 표시하고 로그·내보내기에 보존합니다.
...
재촬영 상태는 빨간색으로 표시하며 Worker 0.2.1은 변경 없이 포함합니다.
...
기준은 `configs/versions/0.2.1.json`이며 Python, Worker, Detector, Embedder, Detector policy,
Classifier policy, Catalog와 사용자 표시 버전은 모두 `0.2.1`입니다. Flutter 내부 빌드만
`0.2.1+24`를 사용합니다.
...
4. 활성 `0.2.1` Runtime은 class-agnostic Detector의 0.025 이상 후보를 이웃 마스크의 context로
   보존하고, `quality.detector_output_score_threshold` 0.04 이상 출력 ROI만
   RepViT-M0.9 192 primary의 정적 batch 2/1로 전달합니다. 생략한 ROI도 context에는 남기며
   context 삭제로 기존 ROI의 판정이 바뀌면 안 됩니다. Detector class를 SKU 승인에 사용하지 않습니다.
5. primary `UNKNOWN`, unsafe, dense scene 저신뢰, 긴 ROI 저신뢰의 전역 조건에 해당하는 ROI만
   별도 DINO Catalog를 사용하는 DINOv3 ConvNeXt-Tiny 224 detail path로 전달하고 Top-3 증거를 병합합니다. 매장별 또는 SKU별
...
있습니다. 현재 `0.2.1`를 독립 일반화 성능, 인증 또는 SLA 달성으로 표현하지 마십시오.
