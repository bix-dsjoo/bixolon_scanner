...
from ..pipeline.decision import softmax as decision_softmax
from ..pipeline.ports import Detection
from ..runtime.onnx import OnnxDetector, OrtRunner, nms, prepare_rgb, sigmoid
...
def _postprocess_detector(logits, boxes, metadata, image_shape) -> list[Detection]:
    ...
    scores = sigmoid(np.asarray(logits)[0]).max(axis=-1)
    detections = []
    ...
    return nms(detections, metadata.nms_iou_threshold)
...
def parity(args: argparse.Namespace) -> None:
    ...
    reference_probabilities = decision_softmax(
        classifier_reference_logits, metadata.classifier.temperature
    )
    onnx_probabilities = decision_softmax(
