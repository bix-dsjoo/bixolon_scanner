...
from ...evaluation.detector import _iou, _metrics, _xywh_to_xyxy
...
def postprocess_worker_gate(
    ...
    detections = nms(accepted_raw, float(options["nms_iou_threshold"]))
    uncertain = 0
    ...
    if uncertainty_threshold is not None:
        ...
        for candidate in shadow:
            ...
            if area_ratio < float(options["uncertainty_min_area_ratio"]):
                continue
            overlaps = [box_iou(candidate, accepted) for accepted in detections]
