...
def _gate_report(scores: np.ndarray, logits: np.ndarray, targets: np.ndarray, threshold: float):
    ...
    return {
        ...
        "accepted_misclassification_count": int(
            (accepted & (targets >= 0) & ~correct).sum()
        ),
    }
...
def main() -> None:
    ...
    model, _checkpoint = _load_checkpoint_model(
        run_dir / "best.pt", config, torch.device("cuda")
    )
    ...
    adapt_labels = (adapt["targets"] >= 0) & (
        adapt["logits"].argmax(axis=1) == adapt["targets"]
    )
    rejector = _fit_rejector(
        adapt["features"], adapt_labels, seed=args.seed, epochs=args.epochs
    )
    risk = extracted["risk"]
    risk_scores = _scores(rejector, risk["features"])
    risk_correct = (risk["targets"] >= 0) & (
        risk["logits"].argmax(axis=1) == risk["targets"]
    )
    ...
    torch.save({"state_dict": rejector.state_dict(), "input_size": adapt["features"].shape[1]}, artifact_dir / "rejector.pt")
    np.savez_compressed(
