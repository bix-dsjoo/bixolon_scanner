...
class ReleaseVersions(BaseModel):
    ...
    detector: str
    classifier: str
    detector_training_pipeline: str
    classifier_training_pipeline: str
...
class ReleaseComposition(BaseModel):
    ...
    training_contracts: TrainingContractComposition
...
def _verify_bundle_manifest(path: Path, composition: ReleaseComposition) -> dict[str, Any]:
    ...
    required = {
        ...
        "worker/model-package/detector.onnx",
        "worker/model-package/classifier.onnx",
    ...
    if (
        sha256_file(root / "worker/model-package/metadata.json")
        != composition.model_package_metadata.sha256
    ):
