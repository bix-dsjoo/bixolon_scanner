...
현재 배포 가능한 실행 조합은 `0.1.8` 하나입니다.

| 구성 | 버전 |
|---|---|
| Python 패키지·Worker | `0.1.8` |
| Detector·Embedder·판정 정책 | `0.1.8` |
| Store Catalog | `0.1.8` |
| Flutter 앱 | `0.1.8+11` |
| 사용자 표시·Windows ProductVersion | `0.1.8` |

기준 설정은 [`configs/versions/0.1.8.json`](configs/versions/0.1.8.json)입니다. 모델 graph·weight와
Catalog payload는 고정하고 Runtime·Catalog metadata의 공개 실행 버전만 하나의 제품 버전으로
맞춥니다. 과거 운영 `0.1.7` 계약은 `configs/archive`와 `docs/archive`에 보존합니다.

`0.1.8`은 `0.1.7`의 모델 graph·weight, Catalog payload, detector threshold와 공개 API를
유지하면서 classifier 안전 정책 하나를 활성화한 patch입니다. 승인되지 않은 `UNKNOWN` ROI에서
회전·독립 verifier가 모두 품질 실패를 반환하면 Top-3를 노출하지 않고 해당 segmentation을
`SEGMENT_RECAPTURE`로 처리합니다. Windows OpenVINO 구성과 GPU 초기화 실패 시의 명시적 CPU
fallback은 그대로 유지합니다.

보호 데이터 415+69장에서는 `0.1.7` 대비 status·reason·bbox·item status·prediction·Top-3 차이가
0이었고, confidence-only 차이는 각각 13장과 62장, 최대 `3.40e-6`이었습니다. 2026-08-28의
`bread_04` Top-3 정답 누락 1건은 `SEGMENT_RECAPTURE`로 바뀌었습니다. 같은 10장의 detector 누락
8개는 남아 있습니다. 이 자료는 개발·비열화 방지 회귀이며 독립 일반화 성능이나 인증이 아닙니다.
상세 내용은 [0.1.8 개발 검증 보고서](docs/evaluation/scanner-0.1.8.md)를 참조하십시오.
...
classifier·Catalog 계열 버전은 `null`입니다. 나머지 공개 non-null 버전은 모두 `0.1.8`입니다.
공개 필드와 판정 순서는 [API 계약](docs/contracts/api.md)을 따릅니다.

Runtime의 선택적 classifier verifier 계약은
`unknown_recapture_on_dual_verifier_rejection`을 지원합니다. schema 기본값은 기존 package 호환을
위해 `false`이고, 활성 `0.1.8` Runtime은 이를 `true`로 고정합니다. 승인 차단 ROI의 회전·독립
verifier가 모두 품질 실패를 반환할 때 기존 공통 `SEGMENT_RECAPTURE` 경로를 사용합니다.
...
.\scripts\build_n100_gpu_test.ps1 -Version 0.1.8 -ReuseBuildEnvironment -Force
.\scripts\build_windows_installer.ps1 -Version 0.1.8 -Force
```

첫 명령은 source manifest와 평가 증빙의 고정 해시를 검증하고 CUDA 포함 Flutter 앱 번들을
`artifacts/versions/0.1.8/bixolon-bakery-ai-scanner-0.1.8`에 만듭니다. 두 번째 명령은 Intel GPU
plugin과 CPU fallback을 포함한 OpenVINO Worker를 구성하며, N100 이름은 장비 진단 패키지에만
사용됩니다. 마지막 명령은 일반 Windows 제품 배포물을 생성합니다.

`-DeviceMatrixPath`는 선택 사항입니다. 지정하면 선택한 `0.1.8` 설정에 SHA-256이 고정되고 같은
Runtime·Catalog를 측정한 진단만 허용합니다. 생략하면 설치 생성은 계속되지만 provenance에
`NOT_RUN_FOR_SELECTED_VERSION`을 기록하며 N100 실측이나 지연 성능을 주장하지 않습니다.

- `artifacts/installers/0.1.8/BixolonBakeryAIScanner-0.1.8-Setup.exe`
- `artifacts/installers/0.1.8/BixolonBakeryAIScanner-0.1.8-Worker.zip`
...
이전 `0.1.5` N100 100장 진단에서 하이브리드 worker 평균/p95는 `440.767/729.284ms`, CPU-only는
...
```powershell
.\scripts\build_worker_handoff.ps1 -Version 0.1.8
```

결과는 `artifacts/handoff/0.1.8/bixolon-worker-0.1.8-windows-x64-openvino.zip`입니다. 고정 N100
측정 스크립트는 제품명이 아니라 하드웨어 진단 도구로만 포함됩니다.

준비된 metadata와 번들을 다시 검증하려면 다음 명령을 사용합니다.

```powershell
bixolon bundle verify --config configs/versions/0.1.8.json
```
...
configs/versions/               활성 제품 0.1.8 설정 하나
