...
현재 배포 가능한 실행 조합은 `0.1.3` 하나입니다.

| 구성 | 버전 |
|---|---|
| Python 패키지·Worker | `0.1.3` |
| Detector·Embedder·판정 정책 | `0.1.3` |
| Store Catalog | `0.1.3` |
| Flutter 앱 | `0.1.3+6` |
| 사용자 표시·Windows ProductVersion | `0.1.3` |

`0.1.3`은 인증이나 SLA 상태가 아니라 함께 실행되는 파일 조합의 식별자입니다. 앱, Worker,
Runtime과 Catalog 내용이 바뀌어 새 EXE를 배포할 때만 patch version을 올립니다. 학습과
평가를 반복하는 동안에는 제품 버전을 바꾸지 않습니다.

기준 설정은 [`configs/versions/0.1.3.json`](configs/versions/0.1.3.json)입니다. Detector는
...
현재 개발 회귀 결과와 데이터 한계는
[0.1.3 개발 검증 보고서](docs/evaluation/scanner-0.1.3.md), 과거 버전과 당시 판단은
...
`0.1.3`입니다. `IMAGE_RECAPTURE`와 `SEGMENT_RECAPTURE`는 각각 하나의 공개 공통 reason만
...
2. 모든 실행 구성요소를 `0.1.3`으로 맞추고 Catalog를 무키 `CHECKSUM-SHA256` 형식으로 구성합니다.
...
결과는 `artifacts/versions/0.1.3/bixolon-scanner-0.1.3`에 생성됩니다. 설치 PC의 Python이나
...
```powershell
.\scripts\build_app.ps1 -Version 0.1.3
.\scripts\build_worker_handoff.ps1 -Version 0.1.3 -ReuseBuildEnvironment
.\scripts\build_n100_test_candidate.ps1 -Version 0.1.3 -Force
.\scripts\build_n100_gpu_test.ps1 -Version 0.1.3 -ReuseBuildEnvironment -Force
```

`artifacts/handoff/n100-0.1.3-candidate-*.zip`은 CPU-only 기준 진단에 사용합니다.

CPU와 Intel 내장 GPU를 직접 비교하려면
`artifacts/handoff/n100-0.1.3-openvino-cpu-gpu-test.zip`을 N100에 풀고
...
`n100-0.1.3-openvino-device-matrix.json` 하나에 응답 parity, 평균·p50·p95·p99, 단계별 지연,
...
```powershell
.\scripts\build_n100_installer.ps1 -Version 0.1.3 `
  -N100DeviceMatrixPath D:\n100-0.1.3-openvino-device-matrix.json -Force
```

N100 device matrix에서 하이브리드 profile 초기화, 오류 0건, semantic parity와 CPU-only 대비 평균·p95
개선이 확인되어야 합니다. 300ms 기준은 개발 PC 기준이며 N100 설치 생성 조건으로 사용하지 않습니다.
활성 `0.1.3`과 같은 모델·Catalog·판정 정책으로 측정한 N100 결과만 참조 진단으로 사용할 수
있습니다. provenance에는 측정 환경과 원본 결과 SHA-256을 기록합니다. 빌드가 완료되면
`artifacts/installers/0.1.3/BixolonScanner-N100-0.1.3-Setup.exe`와 외부 `.sha256`,
`installer-manifest.json`, 그리고 독립 실행용
`BixolonScanner-N100-0.1.3-Worker.zip`과 `.sha256`이 생성됩니다. 두 산출물은 같은 GPU 포함 Worker,
...
```powershell
.\scripts\build_worker_handoff.ps1 -Version 0.1.3
```

결과는 `artifacts/handoff/0.1.3/bixolon-worker-0.1.3-windows-x64-openvino.zip`과 외부
...
```powershell
bixolon bundle verify --config configs/versions/0.1.3.json
```
...
configs/versions/               활성 제품 `0.1.3` 기준 설정 하나
