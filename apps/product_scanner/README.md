...
기본 Worker 주소는 `http://127.0.0.1:8000`입니다. Windows 앱을 실행하면 Worker도 숨김 자식 프로세스로 자동 실행되며, 모델 warm-up이 끝나기 전에 분석을 눌러도 readiness가 열릴 때까지 기다립니다. 앱이 직접 시작한 Worker는 앱 종료 시 함께 종료됩니다. 이미 같은 주소에 Worker가 실행 중이면 `/health/ready`의 모든 non-null 구성요소 버전이 앱 계약 `0.1.2`와 일치할 때만 사용하고, 다른 버전 Worker에는 스캔을 전송하지 않습니다.
...
단일 명령은 `configs\versions\0.1.2.json`의 고정 해시를 확인하고 Runtime/Catalog metadata를
`0.1.2`로 변환한 뒤 Worker, CUDA 13·cuDNN 9 DLL과 Flutter `0.1.2+4`를 자체 포함 Windows
번들로 만듭니다. 결과는 `artifacts\versions\0.1.2\bixolon-scanner-0.1.2`입니다. 설치 PC의
...
NVIDIA GPU가 없는 Intel N100 PC에는 `scripts\build_n100_installer.ps1`로 만든 CPU 전용 Setup
EXE를 사용합니다. 이 설치 이미지에는 같은 `0.1.2` Flutter 앱·Runtime·Catalog와 CPU 전용
ONNX Runtime Worker가 들어가며 CUDA 파일을 포함하지 않습니다. 시작 메뉴 바로가기는
`BIXOLON_PROVIDER=cpu`, embedder `4 threads`와 N100 현장 결과에서 선택된 detector profile을
명시합니다. 대상 PC에는 Python·Flutter·CUDA가 필요 없습니다. `0.1.2` N100 측정은 아직 현장
JSON 대기 중이며 평균·p95가 모두 1초 이내인 결과가 없으면 Setup 생성을 중단합니다. 측정값은
성능 보장이나 SLA가 아닙니다. 자세한 생성·검증 방법은 저장소 루트 README의
`Intel N100 PC용 Windows 인스톨러` 절을 따릅니다.

앱 `0.1.2+4`는 `/health/ready`에서 Worker·Detector·Embedder·Detector policy·Classifier
policy·Catalog의 non-null 버전이 모두 `0.1.2`인지 확인합니다. 대표 상태는 `test/goldens`로도
