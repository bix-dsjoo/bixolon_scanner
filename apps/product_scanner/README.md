...
Scan Log는 최초 AI 판정, Top-3와 confidence, 최종 상품, 사용자 수정 여부 및 확정 방식을 분리해 보존합니다. 상단 `활동`에서 최근 100건을 한국어 상품명·기존 영문 상품명·class ID 또는 Scan ID로 검색하고 입력원·기간·정렬 필터와 상세 판정을 확인할 수 있습니다. 이전 앱에서 영문 상품명으로 저장된 로그도 파일 스키마를 변경하지 않고 현재 카탈로그와 일치하는 항목만 화면에서 한국어로 표시하며, 제거된 과거 상품명은 원문을 유지합니다. 상품 또는 확정 방식이 없는 구버전 기록은 내부 `Unknown` 값을 노출하지 않고 `상품 정보 없음`·`확정 방식 확인 불가`로 안내합니다. 여러 상품 중 영문명이나 class ID로 검색해도 실제 일치한 한국어 상품을 목록 요약의 첫 항목으로 표시합니다. 목록과 상세는 상품·수정 여부를 먼저 보여주고 처리시간·item ID·confidence·확정 방식은 접힌 진단 정보에서 제공합니다. 진단 정보는 60px 헤더와 접힘 상태 Semantics를 제공하며 Tab 포커스 후 Enter·Space로 열 수 있습니다. 저장에 실패하면 원본 입력과 확인 결과를 유지하고 같은 고정 액션 위치에서 `다시 저장`할 수 있습니다.
...
- BIXOLON `X`에서 파생한 대각선은 실제 카메라 또는 입력 이미지가 있어 정렬·분석·재촬영에 도움이 되는 Scan 프리뷰의 `AppScanGuide`에만 사용합니다. 가이드의 `64%` 범위·코너·중앙 표식·선 굵기와 Outline/Accent 색 역할은 `AppDesignTokens`·`AppComponentColors`로 관리합니다. `RECAPTURE`에는 구도 교정을 위해 유지하지만 카메라 확인·복구, `ERROR`, 결과 bounding box 상태에서는 제거하며 Activity·선택·CTA에는 반복하지 않습니다.
...
- 상단 셸과 패널 헤더는 `60px`, 섹션·표 헤더는 `40px`, 일반 컨트롤은 최소 `44px`, Primary action은 `48px`, 목록 행은 `60px`입니다. 확인 대화상자 `400px`, 빈 상태 최대 폭 `360px`, 진단 레이블 `88px`, 포커스 링 `2px`, 선택 외곽선 `1.5px`까지 `AppDesignTokens`를 직접 사용합니다.
- 공통 컴포넌트와 화면의 패딩·간격은 `AppSpacing`의 `4·8·12·16·24·32px` 역할만 사용하며 Theme 버튼·입력 패딩도 같은 토큰에 연결합니다.
- 상태 배지·Brand mark의 compact 크기, 프리뷰 출처 칩 최소 높이, inline·preview·page 진행 표시 크기는 `AppDesignTokens`로 관리합니다. 상태 배지와 출처 칩은 고정 높이에 텍스트를 가두지 않고 큰 글자에서 최소 높이 위로 확장합니다.
...
- Windows 제목 표시줄·작업 표시줄·실행 파일은 Flutter 기본 아이콘 대신 앱 내부와 같은 Orange 포커스 마크를 사용합니다. 공식 BIXOLON 로고를 재제작하지 않으며 `python tool/generate_windows_icon.py --check`로 16–256px ICO 리소스의 최신 상태를 검증합니다.
