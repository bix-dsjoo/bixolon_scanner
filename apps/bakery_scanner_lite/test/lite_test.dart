...
void main() {
  ...
  test(
    'Approved identity survives journal restart and sanitized export',
    () async {
      final raw = response(object: 'APPROVED');
      (raw['segmentations'] as List).first['prediction'] = <String, dynamic>{
        'class_id': 'bread_01',
        'class_name': 'Almond Scone',
        'confidence': .99,
      };
      final result = ScanResult.fromWorker(raw);
      expect(result.objects.single.prediction!.className, 'Almond Scone');
      expect(result.objects.single.top3, isEmpty);
      final temp = await Directory.systemTemp.createTemp('lite-approved-');
      addTearDown(() => temp.delete(recursive: true));
      final logs = FileRunLogs(
        Directory('${temp.path}/logs'),
        Directory('${temp.path}/backup'),
      );
      await logs.append(
        RunRecord(
          attemptId: 'approved',
      ...
      final restored = (await restarted.recent()).single;
      expect(restored.result!.objects.single.prediction!.classId, 'bread_01');
      expect(
        restored.result!.objects.single.prediction!.className,
        'Almond Scone',
      );
      final path = '${temp.path}/export.jsonl';
      await restarted.exportTo(path);
      expect(await File(path).readAsString(), contains('Almond Scone'));
      expect(await File(path).readAsString(), isNot(contains('confidence')));
      final legacy = restored.toJson();
      ((legacy['result'] as Map)['segmentations'] as List).first.remove(
        'prediction',
      );
      expect(
        RunRecord.fromJson(legacy).result!.objects.single.prediction,
        isNull,
      );
    },
  );
  test('Approved predictions validate identity and fall back to class ID', () {
    for (final prediction in [
      null,
      {},
      {'class_id': ''},
      {'class_id': 1},
      {'class_id': 'bread', 'class_name': 3},
    ]) {
      final raw = response(object: 'APPROVED');
      (raw['segmentations'] as List).first['prediction'] = prediction;
      expect(() => ScanResult.fromWorker(raw), throwsFormatException);
    }
    final raw = response(object: 'APPROVED');
    (raw['segmentations'] as List).first['prediction'] = {
      'class_id': 'bread_01',
    };
    expect(
      ScanResult.fromWorker(raw).objects.single.prediction!.className,
      'bread_01',
    );
  });
  testWidgets('Approved product is visible in scan and historical results', (
    tester,
  ) async {
    ...
    final c = ScanController(
      FakeInputs(),
      FakeScanner()
        ..value = ScanResult.fromWorker(response(object: 'APPROVED')),
      MemoryLogs(),
    );
    await c.initialize();
    ...
    expect(find.text('승인 상품: Almond Scone'), findsOneWidget);
    expect(find.text('Top-3'), findsNothing);
    await tester.tap(find.text('실행 로그'));
    await tester.pumpAndSettle();
    await tester.tap(find.textContaining('sample.png'));
    await tester.pumpAndSettle();
    expect(find.text('승인 상품: Almond Scone'), findsOneWidget);
  ...
  testWidgets('Old approved journal shows missing identity explicitly', (
    tester,
  ) async {
    final result = ScanResult(
      ...
      status: 'SEGMENTATION',
      reasons: [],
      objects: [ObjectResult('APPROVED', [])],
      processingMs: 1,
      versions: {},
    );
    await tester.pumpWidget(
      MaterialApp(home: Scaffold(body: ResultDetails(result))),
    );
    expect(find.text('승인 상품명 기록 없음'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
