...
def _config() -> dict:
    return {
        ...
        "audit": {},
        "augmentation": {
            "views_per_source": 2, "output_size": 224,
            "background_source": "procedural-neutral-only",
            "detector_in_training": False, "mixup": False, "cutmix": False,
            "super_resolution": False,
        },
        "training": {
            "runtime_support_cache": False, "distillation": False,
            "legacy_classifier_initialization": False,
            "challenger": {"trainable_scope": "backbone.stages[-1]", "full_backbone_finetune": False},
        },
        "evaluation": {},
    }
...
def test_support_loader_derives_arbitrary_label_count_and_rejects_checksum_drift(tmp_path: Path):
    ...
    for category in range(1, 4):
        for shot in range(10):
            records.append({
                "record_type": "classification", "category_id": category,
                "image_path": f"{category}/{shot}.jpg", "image_sha256": f"{category:02d}{shot:02d}".ljust(64, "a"),
                "split": "train_support",
            })
    ...
    metadata.write_text(json.dumps({
        "record_count": 30, "class_count": 3, "shots_per_class": 10,
...
def test_tta_disagreement_reduces_logit_confidence_without_changing_shape():
    ...
    fused, disagreement = fuse_tta_logits(
        primary, secondary, disagreement_weight=4.0
    )
