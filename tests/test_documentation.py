...
def test_documented_versions_match_single_version_source() -> None:
    ...
    version_config = (ROOT / "configs" / "versions" / "0.1.4.json").read_text(encoding="utf-8")

    assert f'__version__ = "{python_version}"' in package_init
    assert python_version == "0.1.4"
    assert '"version": "0.1.4"' in version_config
    assert '"app_build": 7' in version_config
    assert "version: 0.1.4+7" in flutter_pubspec
    assert "`0.1.4+7`" in root_readme
    assert "`0.1.4`" in current_status
    assert "`yolo26-objectness-single3-consensus-targeted-rotation`" in current_status


def test_only_014_is_exposed_as_an_active_product_contract() -> None:
    ...
    assert version_files == {"0.1.4.json"}
    assert example_versions == {"0.1.4"}
    assert integration_specs == {"worker-integration-0.1.4.md"}
    ...
    for path in active_surfaces:
        assert "0.1.5" not in path.read_text(encoding="utf-8"), path
...
def test_windows_bundle_uses_single_version_root() -> None:
    ...
    assert "artifacts/versions/0.1.4" in normalized_cmake
