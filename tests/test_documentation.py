...
def test_documented_versions_match_single_version_source() -> None:
    ...
    version_config = (ROOT / "configs" / "versions" / "0.1.8.json").read_text(encoding="utf-8")

    assert f'__version__ = "{python_version}"' in package_init
    assert python_version == "0.1.8"
    assert '"version": "0.1.8"' in version_config
    assert '"app_build": 11' in version_config
    assert "version: 0.1.8+11" in flutter_pubspec
    assert "`0.1.8+11`" in root_readme
    assert "`0.1.8`" in current_status
...
def test_only_018_is_exposed_as_an_active_product_contract() -> None:
    ...
    assert version_files == {"0.1.8.json"}
    assert example_versions == {"0.1.8"}
    assert integration_specs == {"worker-integration-0.1.8.md"}
    ...
    for path in active_surfaces:
        assert "0.1.8" in path.read_text(encoding="utf-8"), path
...
def test_018_build11_packaged_worker_smoke_passes() -> None:
    evidence = json.loads(
        (ROOT / "docs" / "diagnostics" / "packaged-worker-0.1.8-build11-smoke.json").read_text(
            encoding="utf-8"
        )
    )

    assert evidence["product_version"] == "0.1.8"
    assert evidence["app_build"] == 11
    assert evidence["passes"] is True
    assert evidence["cases"]["ready"]["all_non_null_versions"] == "0.1.8"
    ...
    assert evidence["cases"]["existing_415_regression"]["semantic_prediction_diff_count"] == 0
    assert evidence["cases"]["existing_415_regression"]["confidence_only_diff_image_count"] == 13
    assert evidence["cases"]["corrupt_image"]["status"] == "ERROR"
    assert evidence["cases"]["missing_image"]["status"] == "ERROR"
    assert evidence["cases"]["unsupported_image"]["status"] == "ERROR"
    assert evidence["cases"]["deployment"]["n100_benchmark_status"] == (
        "NOT_RUN_FOR_SELECTED_VERSION"
    )
    assert evidence["cases"]["deployment"]["hardware_reference_result_included"] is False
    assert evidence["cases"]["deployment"]["runtime_safety_flag_enabled"] is True
    assert all(re.fullmatch(r"[0-9a-f]{64}", digest) for digest in evidence["artifacts"].values())


def test_018_source_candidate_n100_measurement_is_pinned_with_limits() -> None:
    evidence = json.loads(
...
def test_windows_bundle_uses_single_version_root() -> None:
    ...
    assert "artifacts/versions/0.1.8" in normalized_cmake
