...
def test_documented_versions_match_single_version_source() -> None:
    ...
    assert python_version == "0.2.1"
    assert '"version": "0.2.1"' in version_config
    assert '"app_build": 24' in version_config
    assert "version: 0.2.1+24" in flutter_pubspec
    assert "`0.2.1+24`" in root_readme
    assert "`0.2.1`" in current_status
    assert "BIXOLON Bakery AI Scanner" in root_readme
    assert "BIXOLON Bakery AI Scanner" in current_status
    assert f"`{json.loads(version_config)['source_candidate']}`" in current_status


def test_only_021_is_exposed_as_an_active_product_contract() -> None:
    ...
    assert version_files == {"0.2.1.json"}
    assert example_versions == {"0.2.1"}
    assert integration_specs == {"worker-integration-0.2.1.md"}
    ...
    for path in active_surfaces:
        assert "0.2.1" in path.read_text(encoding="utf-8"), path
...
def test_windows_bundle_uses_single_version_root() -> None:
    ...
    assert "artifacts/versions/0.2.1" in normalized_cmake
