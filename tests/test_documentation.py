...
def test_documented_versions_match_single_version_source() -> None:
    ...
    assert python_version == "0.2.0"
    assert '"version": "0.2.0"' in version_config
    assert '"app_build": 23' in version_config
    assert "version: 0.2.0+23" in flutter_pubspec
    assert "`0.2.0+23`" in root_readme
    assert "`0.2.0`" in current_status
...
def test_only_020_is_exposed_as_an_active_product_contract() -> None:
    ...
    assert version_files == {"0.2.0.json"}
    assert example_versions == {"0.2.0"}
    assert integration_specs == {"worker-integration-0.2.0.md"}
    ...
    for path in active_surfaces:
        assert "0.2.0" in path.read_text(encoding="utf-8"), path
...
def test_windows_bundle_uses_single_version_root() -> None:
    ...
    assert "artifacts/versions/0.2.0" in normalized_cmake
