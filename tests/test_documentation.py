...
def test_documented_versions_match_single_version_source() -> None:
    ...
    version_config = (ROOT / "configs" / "versions" / "0.1.2.json").read_text(encoding="utf-8")

    assert f'__version__ = "{python_version}"' in package_init
    assert python_version == "0.1.2"
    assert '"version": "0.1.2"' in version_config
    assert "version: 0.1.2+4" in flutter_pubspec
    assert "`0.1.2+4`" in root_readme
    assert "`0.1.2`" in current_status
    assert "`detector415-single3-bias004-detector-corroboration-cpu`" in current_status
...
def test_windows_bundle_uses_single_version_root() -> None:
    ...
    assert "artifacts/versions/0.1.2" in normalized_cmake
