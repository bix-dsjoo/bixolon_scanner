...
def fixture_tree(tmp_path, monkeypatch):
    worker = tmp_path / "artifacts/installers/0.1.18/windows-payload/worker"
    worker.mkdir(parents=True)
    (worker / "bixolon-worker.exe").write_bytes(b"verified Worker")
    evidence = tmp_path / "artifacts/versions/0.1.18/packaged-cpu-smoke.json"
    ...
    monkeypatch.setattr(
        ...
        lambda _: SimpleNamespace(
            metadata=SimpleNamespace(worker_version="0.1.18"),
        ),
    )
    monkeypatch.setattr(
        lite_bundle,
        "load_version_config",
        lambda _: SimpleNamespace(version="0.1.18", app_build=21),
    )
...
def test_lite_payload_preserves_source_and_records_manifest(tmp_path, monkeypatch):
    ...
    payload = tmp_path / "artifacts/lite/0.1.18/payload"
    ...
    assert provenance["version"] == "0.1.18"
...
def test_lite_source_tampering_preserves_existing_output(tmp_path, monkeypatch):
    worker = fixture_tree(tmp_path, monkeypatch)
    output = tmp_path / "artifacts/lite/0.1.18/payload"
...
def test_lite_rejects_wrong_model_version(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="unchanged 0.1.18"):
        lite_bundle.prepare(tmp_path)
