...
def test_production_finalizer_only_changes_allowed_runtime_metadata(tmp_path) -> None:
    ...
    source.mkdir()
    metadata = {
        "worker_version": "2.0.1-rc.1",
        "promotion_status": "independent_test_pending",
        "detector_policy_version": "2.0.1-rc.1",
        "detector": {"version": "2.0.1-rc.1"},
        "embedder": {"version": "2.0.1-rc.1"},
        "classifier_policy": {"version": "2.0.1-rc.1"},
        "sources": {"embedder": {"training_pipeline_version": "2.0.1-rc.1"}},
    }
    (source / "metadata.json").write_text(json.dumps(metadata), encoding="utf-8")
    (source / "weight.onnx").write_bytes(b"immutable-weight")

    _promote_runtime(source, target, "2.0.1-rc.1")

    promoted = json.loads((target / "metadata.json").read_text(encoding="utf-8"))
    assert promoted["worker_version"] == "2.0.1"
    assert promoted["promotion_status"] == "production"
    assert promoted["detector"]["version"] == "2.0.1"
    assert promoted["embedder"]["version"] == "2.0.1"
    assert promoted["classifier_policy"]["version"] == "2.0.1"
...
def test_production_finalizer_resigns_only_catalog_metadata(tmp_path) -> None:
    ...
    source.mkdir()
    metadata = {
        "catalog_version": "2.0.1-rc.1",
        "embedder_version": "2.0.1-rc.1",
        "classifier_policy_version": "2.0.1-rc.1",
    }
    ...
    assert set(promoted.values()) == {"2.0.1"}
...
def test_owner_waiver_catalog_promotion_keeps_payload_and_uses_checksums_only(
    ...
    promote_catalog_checksum_only(source, target, "2.0.1-rc.3")

    promoted = json.loads((target / "catalog.json").read_text())
    promoted_checksums = json.loads((target / "checksums.json").read_text())
    assert promoted["authentication"] == "CHECKSUM-SHA256"
    assert promoted["catalog_version"] == "2.0.1"
    assert promoted["embedder_version"] == "2.0.1"
    assert promoted["classifier_policy_version"] == "2.0.1"
    assert promoted_checksums["supports.bin"] == checksums["supports.bin"]
    assert promoted_checksums["catalog.json"] == sha256_file(target / "catalog.json")
    assert not (target / "signature.json").exists()


def test_owner_waiver_must_name_every_skipped_gate() -> None:
    waiver = {
        "schema_version": "2.0",
        "release": "2.0.1",
        "source_release_candidate": "2.0.1-rc.3",
        ...
        "failures": sorted(EXPECTED_WAIVED_GATES),
        "waivers": [{"gate": gate} for gate in sorted(EXPECTED_WAIVED_GATES)],
    }

    validate_owner_waiver(
        waiver,
        release_candidate="2.0.1-rc.3",
        runtime_manifest_sha256="a" * 64,
        catalog_manifest_sha256="b" * 64,
    )
