...
def test_ten_shot_manifest_locks_slots_labels_and_provenance(tmp_path: Path):
    ...
    assert {record["source"] for record in records} == {"bread_project_3_ten_shot"}
    assert {record["class_id"] for record in records} == {"bread_01", "bread_02"}
    assert (
        len({(record["category_id"], record["side"], record["view"]) for record in records}) == 20
    )
...
def test_ten_shot_manifest_is_deterministic_and_writes_checksum(tmp_path: Path):
    ...
    first_version = write_ten_shot_manifest(root, first, labels_metadata=labels, expected_classes=2)
