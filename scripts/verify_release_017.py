...
def main() -> None:
    ...
    components = [
        (root / "artifacts/installers/0.1.17/windows-payload", "installer-payload-manifest.json"),
        (
            root / "artifacts/installers/0.1.17/BixolonBakeryAIScanner-0.1.17-Worker",
            "worker-manifest.json",
        ),
        (
            root / "artifacts/external-sdk/1.1.1/BIXOLON-Scanner-SDK-Windows-x64-1.1.1",
            "integration-manifest.json",
        ),
        (
            root
            / "artifacts/store-models/three_bakery/0.1.17/BIXOLON-Store-Model-three_bakery-0.1.17",
            "bundle-manifest.json",
        ),
    ]
    for path, name in components:
        checked[path.name] = {"manifest_passed": True, "file_count": check_manifest(path, name)}
    verify_lite(root / "artifacts/lite/0.1.17/payload")
    for provider, worker in (
        ("cpu", root / "artifacts/installers/0.1.17/windows-payload/worker"),
        ("cuda", version / "bixolon-bakery-ai-scanner-0.1.17/worker"),
    ):
        ...
        json.dumps(
            {
                ...
                "sdk_version": "1.1.1",
                "source_candidate": "ssdlite-margin-dense-20260908",
                "files": files,
