...
if (Test-Path -LiteralPath $targetBundle) {
    if (-not $Force) {
        throw "Version bundle already exists: $targetBundle. Use -Force to replace it safely."
    }
    $previousBundle = Join-Path $versionRoot (".previous-" + [System.IO.Path]::GetRandomFileName())
    [System.IO.Directory]::Move($targetBundle, $previousBundle)
}
...
try {
    ...
    $bundleManifestHash = (
        Get-FileHash -Algorithm SHA256 -LiteralPath $bundleManifest
    ).Hash.ToLowerInvariant()
    if ($null -ne $previousBundle -and (Test-Path -LiteralPath $previousBundle)) {
    ...
    if ($null -ne $previousBundle -and (Test-Path -LiteralPath $previousBundle)) {
        ...
        [System.IO.Directory]::Move($previousBundle, $targetBundle)
    }
    throw
}
