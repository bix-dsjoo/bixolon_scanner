"""Advance active release surfaces while preserving the 0.1.17 contracts verbatim."""
...
def main() -> None:
    ...
    for relative, archived in (
        ("configs/versions/0.1.17.json", "configs/archive/versions/0.1.17.json"),
        (
            "docs/contracts/worker-integration-0.1.17.md",
            "docs/archive/contracts/worker-integration-0.1.17.md",
        ),
        ("docs/contracts/examples/0.1.17", "docs/archive/contracts/examples/0.1.17"),
    ):
    ...
    archived_status = root / "docs/archive/status/0.1.17.md"
    if not archived_status.exists():
        shutil.copy2(root / "docs/status/current.md", archived_status)
    examples = root / "docs/contracts/examples/0.1.18"
    if not examples.exists():
        shutil.copytree(root / "docs/archive/contracts/examples/0.1.17", examples)
    integration = root / "docs/contracts/worker-integration-0.1.18.md"
    if not integration.exists():
        shutil.copy2(root / "docs/archive/contracts/worker-integration-0.1.17.md", integration)
    paths = [
        "pyproject.toml",
        "AGENTS.md",
        "README.md",
    ...
    for path in sorted(selected):
        ...
        new = old.replace("0.1.17+20", "0.1.18+21").replace("0.1.17", "0.1.18")
        if path.name in {
            "README.md",
            "README-KO.md",
            "pubspec.yaml",
            ...
            new = new.replace("1.1.1", "1.1.2")
        if path.name == "test_documentation.py":
            new = new.replace("test_only_0117_", "test_only_0118_").replace(
                '"app_build": 20', '"app_build": 21'
            )
        if path.name == "test_lite_bundle.py":
            new = new.replace("app_build=20", "app_build=21")
        if old != new:
            path.write_text(new, encoding="utf-8")
    print("Active 0.1.18 surfaces updated; 0.1.17 contracts archived.")
