...
def _train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("DINOv3 Faster R-CNN training requires CUDA")
    if args.elongated_repeat < 0 or args.elongated_aspect_ratio <= 1:
        raise ValueError("elongated sampling settings are invalid")
    ...
    synthetic = CachedObjectnessDataset(
    ...
    elongated = Subset(
        ...
        [
            index
            for index, row in enumerate(synthetic.records)
            if any(
                max(
                    annotation["bbox_xywh"][2] / max(annotation["bbox_xywh"][3], 1e-6),
                    annotation["bbox_xywh"][3] / max(annotation["bbox_xywh"][2], 1e-6),
                )
                >= args.elongated_aspect_ratio
    ...
    parts.extend(elongated for _ in range(args.elongated_repeat))
    loader = DataLoader(
        ConcatDataset(parts),
    ...
    trainable = detector_parameters + backbone_parameters
    parameter_groups = [{"params": detector_parameters, "lr": args.learning_rate}]
    ...
    report = {
        "schema_version": "1.0",
        "experiment": "store2_source_only_dinov3_convnext_tiny_fasterrcnn",
        ...
        "settings": {
            ...
            "elongated_aspect_ratio": args.elongated_aspect_ratio,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a fresh source-only DINOv3 Faster R-CNN detector"
    )
    ...
    parser.add_argument("--empty-repeat", type=int, default=16)
    parser.add_argument("--elongated-repeat", type=int, default=0)
    parser.add_argument("--elongated-aspect-ratio", type=float, default=2.0)
