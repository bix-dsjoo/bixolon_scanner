param(
    [string]$Composition = "configs/releases/scanner_2.0.0.json",
...
function Resolve-BuildExecutable {
    ...
    if ($ConfiguredPath) {
        ...
        $configuredCommand = Get-Command $ConfiguredPath -CommandType Application `
            -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($null -eq $configuredCommand) {
            throw "Configured build executable was not found: $ConfiguredPath"
        }
    ...
    if ($null -eq $command) {
        throw "Required build executable was not found on PATH: $Name"
    }
...
$FlutterExecutable = Resolve-BuildExecutable -Name "flutter" -ConfiguredPath $FlutterExecutable
$PythonExecutable = Resolve-BuildExecutable -Name "python" -ConfiguredPath $PythonExecutable
...
if ($isScannerV2) {
    ...
    if ($attestation.release -ne "2.0.0" -or
        ...
        throw "Scanner 2.0 production attestation does not match the composition."
    }
    ...
    if ($catalog.authentication -ne "CHECKSUM-SHA256" -or
        ...
        throw "Scanner 2.0 Release requires a keyless checksum-only Catalog."
    }
    ...
    if ($workerSha256 -ne $compositionValue.production_release.worker_executable_sha256) {
        throw "Standalone Worker checksum does not match the Scanner 2.0 composition."
    }
}
else {
